...
def main():
    ...
    print("\n We are sparse training the baseline for these many epochs: ", epochs)

    device_for_loading_lth = torch.device('cpu')

    model_A_dense_init_for_LTH = resnet50_wide().to(device_for_loading_lth)
    load_path_rewound_point_A_for_LTH = f"/scratch/rjain/sparse_training/wide_resnet50_cp/model_a/model_{k}.pth"

    checkpoint_1 = torch.load(load_path_rewound_point_A_for_LTH, map_location=device_for_loading_lth)
    state_dict_1 = {k.replace("module.", ""): v for k, v in checkpoint_1.items()}


    model_A_dense_init_for_LTH.load_state_dict(state_dict_1)
    ...
    original_model_A_sparse = torch.load(model_A_sparse_path, map_location=device_for_loading_lth)

    transfer_sparsity_resnet(original_model_A_sparse, model_A_dense_init_for_LTH)
    check_hooks(model_A_dense_init_for_LTH)
    print("Sparsity of the rewound init after transfer sparsity: ",calculate_overall_sparsity_from_pth(model_A_dense_init_for_LTH))
    ...
    torch.save(model_A_dense_init_for_LTH, f'lth_feb19_rewind_at_{args.rewind_epoch}_sparsity_{sparsity_for_printing}')
...
def main_worker(gpu, ngpus_per_node, args, sparse_epochs):
    ...
    if gpu == 0:
        ...
        wandb.init(
        project="rohan_resnet50_imagenet_lth",
            name=f"resnet50_lth_{args.rewind_epoch}_sparsity_{sparsity_for_printing}; {current_time}",
            config={
    ...
    if args.sparsity == 7:
        ...
        sparsity = 97
    model = torch.load(f'lth_feb19_rewind_at_{args.rewind_epoch}_sparsity_{sparsity}') # model is being passed as an arg now
    ...
    if gpu == 0:
        ...
        # torch.save(model, "/project/def-yani/rjain/sparse-rebasin/sparse-rebasin/sparse_solutions_w2_sparsity_90/LTH_solution_w2_sparsity_90.pth")
...
def validate(val_loader, model, criterion, args):
    ...
    # wandb.log({"test_acc (top1)": top1.avg})
