...
### NAIVE (FILE PATHS ARE MINNEWANKA)
...
def main():
    ...
    k = args.rewind_epoch
    value_for_path = args.sparsity
    epochs = dense_model_trained_for_these_many_epochs - k
    
    print("\n We are sparse training the naive for these many epochs: ", epochs)

    device_for_loading_naive = torch.device('cpu')

    model_B_dense_init_for_naive = resnet50_wide().to(device_for_loading_naive)
    path_rewound_point_B_for_naive_and_permuted = f"/scratch/rjain/sparse_training/wide_resnet50_cp/model_b/model_{k}.pth"

    checkpoint_1 = torch.load(path_rewound_point_B_for_naive_and_permuted,  map_location=device_for_loading_naive)
    state_dict_1 = {k.replace("module.", ""): v for k, v in checkpoint_1.items()}


    model_B_dense_init_for_naive.load_state_dict(state_dict_1)

    model_A_sparse_path = f"/scratch/rjain/sparse_training/wide_resnet50_cp/final_mask_A/model_level_{value_for_path}_epoch_19.pth"
    original_model_A_sparse = torch.load(model_A_sparse_path, map_location=device_for_loading_naive)

    transfer_sparsity_resnet(original_model_A_sparse, model_B_dense_init_for_naive)
    check_hooks(model_B_dense_init_for_naive)
    print("Sparsity of the rewound init after transfer sparsity: ",calculate_overall_sparsity_from_pth(model_B_dense_init_for_naive))
    ...
    torch.save(model_B_dense_init_for_naive, f'naive_feb19_rewind_at_{args.rewind_epoch}_sparsity_{sparsity_for_printing}')
...
def main_worker(gpu, ngpus_per_node, args, sparse_epochs):
    ...
    if gpu == 0:
        ...
        wandb.init(
        project="rohan_resnet50_imagenet_naive",
            name=f"resnet50_naive_{args.rewind_epoch}_sparsity_{sparsity_for_printing}; {current_time}",
            config={
    ...
    if args.sparsity == 7:
        ...
        sparsity = 97
    model = torch.load(f'naive_feb19_rewind_at_{args.rewind_epoch}_sparsity_{sparsity}') # model is being passed as an arg now
    ...
    if gpu == 0:
        ...
        # torch.save(model, "/project/def-yani/rjain/sparse-rebasin/sparse-rebasin/sparse_solutions_w2_sparsity_90/Naive_solution_w2_sparsity_90.pth")
